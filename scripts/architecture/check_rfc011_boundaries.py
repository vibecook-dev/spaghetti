...
def discover_rfc012_scoped_host_boundary_violations() -> set[str]:
    ...
    # The RFC 012D TypeScript contract requirements were removed with the
    # modules they pinned. They required `packages/sdk/src/index.ts` to
    # `export *` eleven hand-written wire parsers, which the RFC 012 landing
    # plan §4 and §5.4 reverse on both counts: the SDK barrel is an explicit
    # allowlist, and types cross the boundary generated rather than
    # hand-written. What replaces the parsers is `pnpm generate:types`, with
    # CI diffing `packages/sdk/src/generated`. The Rust-side scoped-host
    # ownership rules above are unchanged.
    return found
