...
import { accentHex } from '../../lib/archive-theme.js';
...
const FilterPill = memo(function FilterPill({
  ...
  return (
    <button
      ...
      aria-pressed={active}
      className="flex h-8 min-w-0 items-center justify-between gap-2 border-0 border-b border-[color:var(--archive-ink-line-strong)] px-1 bg-transparent font-mono text-[11px] tracking-[0.04em] transition-colors cursor-pointer text-ink"
      style={{
    ...
    >
      ...
      <span className="flex shrink-0 items-center gap-1.5 opacity-45">
        ...
        {/* Design mute affordance stays archival (sanguine), not Tailwind red */}
        <span className="p-0.5" style={{ color: state.mute ? 'var(--archive-sanguine)' : undefined }}>
  ...
  return (
    ...
    >
      <div className="flex w-full flex-wrap items-center gap-x-2 gap-y-1.5">
        ...
        {DEFAULT_TYPE_FILTERS.map(({ key, label, kind }) => {
          ...
          .map(([toolName, count]) => {
            ...
            return (
              <FilterPill
                key={toolName}
                label={toolName}
                color={accentHex('tool_use', isDark)}
                count={count}
                state={state}
