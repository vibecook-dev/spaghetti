...
export function LoadingScreen({
  ...
  return (
    <div
      className={`h-full w-full p-4 flex items-center justify-center transition-colors ${
        isDark ? 'bg-[#0a0908] dark text-[#d4cbbd]' : 'bg-[#d1ccc0] text-[#2b2623]'
      }`}
      role="status"
      aria-live="polite"
      aria-busy={busy}
    >
      <div
        className={`w-full max-w-md border shadow-2xl p-8 ${isDark ? 'border-[#d4cbbd]/40' : 'border-[#2b2623]/50'}`}
      ...
      >
        ...
        {error && !retrying && (
          <div className="border border-sanguine/30 p-3 mb-4">
            ...
            {onRetry && (
              <button
                type="button"
                className="mt-3 font-mono text-[9px] tracking-widest uppercase border border-ink px-3 py-1.5 bg-transparent text-ink cursor-pointer hover:bg-ink hover:text-paper transition-colors"
                onClick={() => onRetry()}
                disabled={retrying}
              >
                Delete cache &amp; retry
        ...
        <footer className="flex items-center gap-2 font-mono text-[8px] tracking-widest uppercase opacity-40 pt-2 border-t border-ink/10">
