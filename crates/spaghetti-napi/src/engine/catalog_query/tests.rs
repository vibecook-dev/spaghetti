...
fn rfc012_x1_emit_complete_only_ingest_trace() {
    ...
    use crate::engine::search_query::{read_search_page, SearchPageRequest};

    let started = Instant::now();
    let published = publish_catalog(1, 1);
    let catalog_ms = u64::try_from(started.elapsed().as_millis()).unwrap_or(u64::MAX);
    let search_request = SearchPageRequest {
        text: "anything".to_string(),
        project_id: None,
        session_id: None,
        adapter_ids: Vec::new(),
    ...
    published
        ...
        .unwrap();
    assert!(matches!(
        read_search_page(&published.connection, &search_request),
        Err(crate::engine::EngineError::BootstrapInProgress)
    ));
    let history_ms = u64::try_from(started.elapsed().as_millis()).unwrap_or(u64::MAX);
    let incomplete_ms = history_ms;
    published
        .connection
        .execute(
    ...
    let default_path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../scripts/rfc012_experiments/fixtures/fts-bootstrap-trace.json");
