//! Atomic durable RFC 012B initial Library catalog publication.
//!
//! This module consumes only the checked private B3 publication envelope. It
//! owns no source reads, retention, query execution, N-API, or refresh path.
...
pub(super) fn load_ready_publication(
    ...
    let stored = connection
        .query_row(
            ...
            SELECT build_commit_seq, durable_publication_contract_version,
                   ...
                   readiness_epoch,
                   attempt,
                   CASE WHEN length(contract_selection_json) BETWEEN 1 AND 4194304
                        THEN contract_selection_json END,
    ...
    let snapshot_count: i64 = connection
        ...
        .map_err(|error| catalog_state::sqlite_error("count catalog snapshots", error))?;
    if snapshot_count != 1 {
        return Err(catalog_state::corrupt_catalog_state(
            "initial catalog publication requires exactly one immutable snapshot",
        ));
    }
    ...
    let member_identity_contract_id = match (
        ...
        stored.member_identity_contract_id.as_deref(),
    ) {
        (1, None) => None,
        ...
        .prepare(
            ...
            SELECT ordinal,
                   CASE WHEN entry_kind IN (
                       'source', 'member_binding', 'reducer_state',
    ...
    let publication_digest = CatalogInitialPublicationDigest::from_digest(publication_digest);
    ...
    let content_digest = derive_durable_content_digest(
        crate::catalog_contract::publication::CatalogInitialBuildExpectation {
...
const fn kind_index(kind: CatalogDurablePublicationEntryKind) -> usize {
    match kind {
        ...
        CatalogDurablePublicationEntryKind::ReducerState => 2,
        CatalogDurablePublicationEntryKind::ProjectRow => 3,
        CatalogDurablePublicationEntryKind::SessionRow => 4,
        CatalogDurablePublicationEntryKind::Tombstone => 5,
    }
}
...
mod tests {
    ...
    fn engine_publishes_only_the_new_ready_commit_watermark() {
        ...
        let engine = SpaghettiEngineCore::open(EngineOptions {
            database_path: directory.path().join("catalog-core.db"),
