...
afterEach(async () => {
  ...
  for (const directory of tempDirs.splice(0)) {
    ...
    try {
      ...
      console.warn(`[observation-shadow] temp dir not removed: ${String(error)}`);
    }
  }
});
...
describe('Claude observation shadow native lifecycle', { skip: !native }, () => {
  ...
  test('queries durable runtime presence and bounded team/inbox snapshots', async () => {
    ...
    const runtimeEntries = [...runtime.entries, ...runtimeRest.entries];
    const presence = runtimeEntries.find((entry) => entry.kind === 'presence')?.presence;
    assert.equal(presence?.nativePid, legacyPresence.pid);
    assert.equal(presence?.nativeStatus, legacyPresence.status);
    assert.equal(presence?.nativeSessionId, legacyPresence.sessionId);
    assert.equal(presence?.cwd, legacyPresence.cwd);
