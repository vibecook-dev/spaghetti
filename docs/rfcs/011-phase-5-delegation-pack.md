...
Status: first end-to-end capability slice implemented on 2026-08-11
...
- `Live`, because a terminated child transcript record can be observed through
  the shared append driver without waiting for session completion.
...
The current Claude decoder emits one layout-strength vendor-subagent assertion
from each subagent transcript record. This is replay-safe and leaves stronger
spawn/meta enrichment for later Phase 5 streams.
...
- Claude advertises the pack at the declared quality and only on its subagent
  stream;
...
The full Rust workspace suite contains 331 passing tests after this slice.

## Deferred Phase 5 work

This increment does not switch production live observation yet. The next
delegation enrichment should add subagent meta and native parent spawn
correlation, followed by teams/config/inbox snapshots, active-session presence,
and the task/plan/artifact packs whose semantics pass review. The observation
