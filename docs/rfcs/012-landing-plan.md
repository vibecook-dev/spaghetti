...
| Wave 2 | in progress: **L5b support** (`land/l5b-support`), **L6 surface**, **L7 perf**. Done: **L1e** `168a829` (observer evaluates the declared ScopeProgram — no path shape left in `observer/scope.rs`; `source/locator.rs` is the single locator law, confinement enforced at emission + resolution; hostile identity values refused; 3 behavioral tests on the shipped manifest; 45 pre-existing observer tests unchanged; no JS-visible change), **L5 adapters** `ffc6c2f`, **L8 docs** `4febb6e`, plus L1c/L1d/L4b/L0/CI wiring. Open: observer `lifecycle::truncating_the_transcript_resets_before_replaying_it` flaked once under load (9/9 ×3 idle) — harden like L1c (drive passes deterministically) in a later pass; L1's agent context is exhausted — future observer work needs a fresh agent with `observer/` + `docs/rfcs/012d-*.md` as its brief | Rust 727/727 (3 ignored), SDK 401/0, CLI 117/0, playground 35/0, validate-all 9/9; clippy 106 left, all L5b's files |
| Wave 3 | not started | — |
