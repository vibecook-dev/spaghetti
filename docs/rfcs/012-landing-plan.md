...
| Wave 2 | in progress: **L5d identity (BLOCKER)** (`land/l5d-identity`: real Claude transcripts fail ingest with `UNIQUE constraint failed: fact_records.semantic_fact_revision_id` since the 8 family emitters landed; tiny fixtures pass, which is why CI was green; fix identity derivation + surface startup errors + real-corpus regression tests), **L5c support-dead** (`land/l5c-support-dead`: delete the dead ~1,400-line access-request subsystem in adapter/support.rs + 8 stragglers → clippy zero), **L6b refs** (`land/l6b-refs`: one RFC 012A ref encoding/typing across catalog/durable/observer, named exports of value types, typed BootstrapInProgress error). Done: **L7 perf** `c29ae151` (durable ingest **70 → 11,653 rec/s (166×)** on a 301 MB frozen corpus, full real corpus ~3 h → 202 s; root causes: usage-v2 intern upsert assigned the parent key → FK scan of unindexed children per intern (99% of writer time), and cold builds no longer entered query-bootstrap mode since 08-21; guards fail on old code; bench harness fixed; `docs/rfcs/012-landing-perf-report.md`), **L5b support** `6291174a` (08-15 bundle deleted with coverage preserved as a fixture, one `version` field + dated bundle dirs, probe triplicates collapsed, reducer dead half removed; promotion of real releases is blocked on HUMAN acts: named sanitizer review + performance report — validator refuses placeholders), **L6 surface** `97eb9e0`, **L1e** `168a829`, **L5** `ffc6c2f`, **L8** `4febb6e`, L1c/L1d/L4b/L0/CI | Rust 816–829 tests green, SDK 359/0, CLI 117/0, playground 35/0, validate-all 9/9; Rust crate ~150k LOC, SDK ~54k; clippy 76 left (L5c) |
| Wave 3 | not started | — |
