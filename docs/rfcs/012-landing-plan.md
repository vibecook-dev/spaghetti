...
| Wave 2 | in progress: **L5b support** (`land/l5b-support`: support-release model, promote real releases, delete candidate-2026-08-15, triplicates, last clippy warnings in adapter/support.rs + runtime_semantic_reducer.rs), **L6 surface** (+ externalRef encoding unification), **L7 perf**, **L8 docs** (refresh for L5 then merge). Done: **L5 adapters** `ffc6c2f` — Claude decoder emits all 11 families (`claude/runtime_facts.rs` 1,248 prod/1,041 test), typed `SemanticEvent.value` (`RuntimeSemanticValue`), e2e proof `observe-session-families.test.ts`, decode 24.9 → 19.1 ms/MB (3 families) / 40.6 → 34.2 (11 families) with identical digests (budget ≤10 not reachable by tuning: identity derivation → L7), 9 Claude scope relations restored, Grok usage exact per response, `claude/adapter.rs` split (5,064 hidden prod lines → 2,892 + capabilities/sidecars); plus L1c/L1d/L4b/L0/CI wiring | Rust 719/719 (3 ignored), SDK 401/0, CLI 117/0, playground 35/0, validate-all 9/9; Rust crate 152,644 LOC (+2.7k for the 8 families), SDK 62,530; clippy: ~96 warnings left, all in adapter/support.rs + runtime_semantic_reducer.rs (L5b) |
| Wave 3 | not started | — |
