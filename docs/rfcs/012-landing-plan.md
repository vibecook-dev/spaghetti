...
| Wave 2 | in progress: **L5 adapters** (`land/l5-adapters`), **L0 hygiene** (`land/l0-hygiene`); L6/L7/L8 not started. Done: **L1c** `df5a459` (exact-count observer tests, 16/16 under load), **L1d** `2f83d8b` (confirmed-absent sidecars cost one stat: 574 reads → 0), **L4b** `a36454f` (playground library on the catalog path: **491 ms** to rows on the real corpus vs not within 600 s before; parity 7/7 on fixtures), playground tests wired into `test:packages`/CI (`next commit`) | Wave 1 complete 2026-08-23 12:22: Rust crate 315,610 → 153,324 LOC, SDK 81,349 → 61,977; ratchet baseline 4/24/17. Open: SDK "Grok native cold ingest smoke" suite silently skips (gated on legacy addon exports `ingest`/`liveIngestBatch` that no longer exist) → L6/L7 to re-gate or delete |
| Wave 3 | not started | — |
