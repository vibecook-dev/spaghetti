...
| Wave 2 | in progress: **L5b support** (`land/l5b-support`), **L1e** (`land/l1e-scope-program`: observer evaluates the declared ScopeProgram via the `source/locator.rs` trio L5b extracts), **L6 surface** (+ externalRef encoding unification, + export RuntimeSemanticValue/*Fact types), **L7 perf**. Done: **L5 adapters** `ffc6c2f` (all 11 families emitted, typed value, Grok exact usage, 9 scope relations, decode −16–23%), **L8 docs** `4febb6e` (012B/C/D trimmed to 237/276/274 lines, program docs + 08-15 drafts archived under `docs/rfcs/archive/`, README + CHANGELOG 0.8.0 notes, `docs/integration/{chopsticks-observe-session,vibefield-phase-a}.md`, petition status; live RFC 012 set ≈ 3,000 lines from 12.8k), plus L1c/L1d/L4b/L0/CI wiring | Rust 719/719 (3 ignored), SDK 401/0, CLI 117/0, playground 35/0, validate-all 9/9; Rust crate 152,644 LOC, SDK 62,530; clippy: ~106 warnings left, all in adapter/support.rs + runtime_semantic_reducer.rs (L5b) |
| Wave 3 | not started | — |
