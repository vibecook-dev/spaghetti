...
| Wave 2 | in progress: **L5b support** (`land/l5b-support`), **L6b refs** (`land/l6b-refs`: one RFC 012A ref encoding across catalog/durable/observer + named exports of the value types), **L7 perf**. Done: **L6 surface** `97eb9e0` (−12,563 lines: Engine* DTO mirror collapsed — results cross as JSON + ts-rs types, `napi_engine.rs` 5,915 → 2,790, `native.ts` 2,243 → 804, hand-written TS contracts/parity tests → `engine-json-contract.test.ts` over real engine output, silently-skipping Grok suite now runs 6/6, tail latency getTimeline p95 104 → 64 ms; two persistence formats protected), **L1e** `168a829`, **L5 adapters** `ffc6c2f`, **L8 docs** `4febb6e`, L1c/L1d/L4b/L0/CI | Rust 816/816 (3 ignored), SDK 359/0, CLI 117/0, playground 35/0, validate-all 9/9; Rust crate 150,992 LOC, SDK **53,857**; ratchet 3/19/17; clippy 107 left, all L5b's files |
| Wave 3 | not started | — |
